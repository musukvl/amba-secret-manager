...
- **Profile** — a named set of secret files stored under `~/.secrets/<profile-name>/`
...
- Clean replace: if the profile already exists in `~/.secrets/<profile>/`, delete it entirely before writing
- Copy secrets into `~/.secrets/<profile>/`, preserving the relative folder structure
- No confirmation prompt — just save
- Profile name defaults to the current folder name if not provided
- Example: `sm save` in `~/dev/myapp` → saves to `~/.secrets/myapp/`
- Example: `sm save prod` in `~/dev/myapp` → saves to `~/.secrets/prod/`

## Load profile — `sm load [profile-name]`
- Copy all files from `~/.secrets/<profile>/` back into the current directory, preserving folder structure
...
- Example: `sm load` in `~/dev/myapp` → loads from `~/.secrets/myapp/`
- Example: `sm load prod` in `~/dev/myapp` → loads from `~/.secrets/prod/`

## List profiles — `sm list`
- List all profile names stored in `~/.secrets/` (each subdirectory is a profile)
